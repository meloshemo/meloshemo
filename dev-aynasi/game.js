...
(function () {
  ...
  function drawReflection(p, seg, kind, distortion) {
    ...
    if (isGiant) {
      ...
      drawFigure(rx, ry, sx, sy, alpha, `rgba(${C.glass}, 0.92)`, null);
    }
    ctx.restore();
    return true;
  }
  ...
  window.__devAynasi = {
    ...
    reflectionCheck() {
      const p = players[0];
      const g = segGeometry(giant);
      const yatay = giant.horizontal;
      const eskiX = p.x;
      const eskiY = p.y;
      const olc = (dx, dy) => {
        p.x = (yatay ? g.ax + CELL / 2 : g.ax + CELL * 0.4) + dx;
        p.y = (yatay ? g.ay + CELL * 0.4 : g.ay + CELL / 2) + dy;
        const t = yatay ? (p.x - g.ax) / CELL : (p.y - g.ay) / CELL;
        const dik = yatay ? p.y - g.ay : p.x - g.ax;
        const uzak = Math.abs(dik);
        ...
        return {
          onunde: true,
          // figür bandın içinde tamamen duruyor mu
          sigiyor: derinlik + figurYari <= bandDerin + 0.01,
          // oyuncuyla çakışmıyor mu (yansıma merkezi ile oyuncu arası)
          cakismiyor: derinlik + uzak >= figurYari * 0 + RADIUS + figurYari * 0.35,
          derinlik: Math.round(derinlik), figur: Math.round(figurYari * 2),
        };
      };
      const sonuc = {
        tamKarsi: olc(0, 0),
        uzak: olc(yatay ? 0 : 60, yatay ? 60 : 0),
        yanda: olc(yatay ? 55 : 0, yatay ? 0 : 55),
        panelDisi: olc(yatay ? 130 : 0, yatay ? 0 : 130),
      };
      p.x = eskiX;
      p.y = eskiY;
      return sonuc;
