...
(function () {
  ...
  // Labirentin DOKUSU da bölümle birlikte sertleşir. Ölçüm şunu gösterdi:
  // doku toplam koridor uzunluğunu neredeyse hiç değiştirmiyor (%4), yani
  ...
  const CHAPTERS = [
    ...
    // --- Dördüncü perde: imkansıza yakın dört salon ---
    {
    ...
    {
      // XVIII · Reykjavík — Buz Salonu: zemin kaygan. Durmak da dönmek de
      // zor; aynanın önünde durabilmek başlı başına iş.
      size: 68, light: 225, hints: 3, decoys: 32,
      ice: { friction: 0.1, accel: 0.42 },
      palette: { ground: "#050a10", floorA: "#0e1a26", floorB: "#0a141e", glass: "186, 226, 255", lamp: "214, 240, 255" },
    },
    {
      // XIX · Marrakeş — Sönen Fener: fener sürekli sönüyor. Yalnızca yeni
      // cam gördükçe yağ alıyor; aynı yerde dönersen karanlıkta kalırsın.
      size: 68, light: 245, hints: 3, decoys: 34,
      lantern: { decay: 0.05, gain: 0.013, floor: 0.3 },
      palette: { ground: "#0d0705", floorA: "#1c120a", floorB: "#150d07", glass: "244, 196, 132", lamp: "255, 214, 156" },
    },
    {
      // XX · Sonsuzluk — Yalancı Aynalar: birkaç cam dev görüntü verir ama
      // yalan söyler. Gerçek ayna simetriktir: yana yürüdüğünde yansıma da
      // seninle kayar. Yalancının görüntüsü panelin ortasında çakılı kalır.
      // Üstelik gerçek ayna hâlâ kaçıyor.
      size: 74, light: 212, hints: 4, decoys: 38, liars: 5,
      fleeing: { every: 26000, range: CELL * 3.0, grace: 1400 },
      returnTrip: true, fading: true,
      palette: { ground: "#06060c", floorA: "#171724", floorB: "#10101a", glass: "228, 228, 248", lamp: "236, 236, 255" },
    },
  ];
    ...
    return Object.assign(
      {
        ...
        decoyBin: Math.min(14, 5 + n),   // her 1.000 aynada kaç sahte dev
  ...
  function newRoom(nextSeed) {
    ...
    seed = nextSeed;
    const doku = endless
      ? dokuIcin(Math.min(endlessCount, 19), 20)
      : dokuIcin(chapterIndex, CHAPTERS.length);
    ...
    if (chapter.decoys) {
      ...
      const rand = () => ((s = (s * 1664525 + 1013904223) >>> 0) / 4294967296);
      let guard = 0;
      // Sonsuz modda yoğunluk doğrudan ayna sayısından hesaplanır: oradaki
      // chapter.size yalnızca bir referans olduğu için alan ölçeği zıplıyordu.
      const hedefDecoy = chapter.decoyBin
        ? Math.round((mirrorCount * chapter.decoyBin) / 1000)
        : Math.round(chapter.decoys * alanOlcek);
    ...
    // Yalancı aynalar: dev görüntü verirler ama simetriye uymazlar.
    liars = new Set();
    if (chapter.liars) {
      const all = Maze.mirrors(maze);
      ...
      while (liars.size < hedefLiar && guard++ < 60000) {
        ...
        if (seg.id === giant.id) continue;
        const c = segCenter(seg);
        if (Math.hypot(c.x - giantSeg.x, c.y - giantSeg.y) < CELL * 6) continue;
        liars.add(seg.id);
      }
    }
    gorulenSayac = 0;
  ...
  function moveAll(dt) {
    ...
    for (const p of players) {
      ...
      // Buz salonu: ivme de sürtünme de düşer, oyuncu kayar.
      const buz = chapter.ice;
      const ivme = ACCEL * (buz ? buz.accel : 1);
      const surtunme = FRICTION * (buz ? buz.friction : 1);
      ...
      if (len) {
        ...
        const hiz = Math.hypot(p.vx, p.vy);
        const yeni = Math.max(0, hiz - surtunme * dt);
        if (hiz > 0) {
          p.vx = (p.vx / hiz) * yeni;
          p.vy = (p.vy / hiz) * yeni;
        }
    ...
    for (const p of players) durgunlukIzle(p);
  }
  ...
  function yansimaGeometrisi(p, seg, kind, distortion) {
    ...
    const menzil = buyuk ? GIANT_REVEAL : YANSIMA_MENZIL;
    if (uzak > menzil) return { cizilir: false, sebep: "menzil" };

    // Panel boyunca konum: gerçek simetride oyuncuyla aynı hizadadır.
    // Yalancı ayna simetriye uymaz: görüntüsü panelin ortasına çakılıdır,
    // yana yürüdüğünde seninle birlikte kaymaz. Tek ve adil ipucu budur.
    const eksen = isLiar ? eksenBas + CELL / 2 : (yatay ? p.x : p.y);
    const sx = (buyuk ? GIANT_SCALE : 1) * (distortion ? distortion.sx : 1);
    const sy = (buyuk ? GIANT_SCALE : 1) * (distortion ? distortion.sy : 1);
    ...
    if (gorunen < (buyuk ? 0.35 : 0.55)) return { cizilir: false, sebep: "tasma", gorunen };
    ...
    return {
      cizilir: true, isGiant, isLiar, yatay, taraf, duzlem, eksenBas, eksen,
      uzak, derinlik, rx, ry, sx, sy, yariGenis, yariBoy, gorunen, band, alpha,
    };
  }
  ...
  //
  //  1) Yansıma, oyuncunun ayna düzlemine göre TAM SİMETRİĞİDİR. Panel boyunca
  //     oyuncuyla aynı hizada, camın arkasında aynı uzaklıkta durur.
  //  2) Yansımanın boyu değişmez; yalnızca uzaklaştıkça soluklaşır.
  //  3) Bir aynada ancak onun önündeysen görünürsün: figür panelden büyük
  //     ölçüde taşacaksa o ayna hiç görüntü vermez — yarım figür çizilmez.
  //  4) Kırpma yalnızca panelin genişliği ve camın arkası ile yapılır;
  //     derinlikte kesme yoktur, yani figür ortadan bölünmez.
  //  5) Yansıma oyuncunun üstüne binmez: cama çok yaklaşıldığında görüntü
  //     yumuşakça geriye itilir.
  //  6) Tek istisna XX. bölümün yalancı aynasıdır; kural (1)'i bilerek çiğner
  //     ve oyuncunun onu ayırt etmesi beklenir.
  function drawReflection(p, seg, kind, distortion) {
    const y = yansimaGeometrisi(p, seg, kind, distortion);
    if (!y.cizilir) return false;
    const band = y.band;
    ...
    const g0 = y.yatay
      ? { x: band.x, y: y.taraf > 0 ? band.y + band.h : band.y }
      : { x: y.taraf > 0 ? band.x + band.w : band.x, y: band.y };
    const g1 = y.yatay
      ? { x: band.x, y: y.taraf > 0 ? band.y : band.y + band.h }
      : { x: y.taraf > 0 ? band.x : band.x + band.w, y: band.y };
    ...
    if (y.isGiant) {
      drawFigure(y.rx, y.ry, y.sx, y.sy, 1, "#ffd98a", "rgba(216, 178, 106, 0.9)");
    } else if (y.isLiar) {
      drawFigure(y.rx, y.ry, y.sx, y.sy, 1, "#e8d9a6", "rgba(216, 178, 106, 0.55)");
    } else {
      drawFigure(y.rx, y.ry, y.sx, y.sy, y.alpha, `rgba(${C.glass}, 0.92)`, null);
    }
    ctx.restore();
    return true;
  }
  ...
  // Sonsuz oda kartı: o odada yürürlükte olan kuralların adını da yazar.
  function sonsuzKartMetni() {
  ...
  function enterMirror(now) {
    if (endless) {
      ...
      els.cardText.textContent = sonsuzKartMetni();
  ...
  function loop(now) {
    ...
    if (running && !finished && !carding) {
      ...
      if (chapter.lantern) {
        ...
        fade = Math.max(chapter.lantern.floor, fade - dt * chapter.lantern.decay);
      }
      if (chapter.water) {
  ...
  function begin(isDuel, nextSeed, bolum = 0, sonsuz = false) {
    ...
    newRoom(nextSeed);
    // Sonsuz modun ilk odası da kuralını duyursun: sonraki odalar kartı
    // enterMirror'da alıyordu, ilki hiç almıyor ve oyuncu odaya kör giriyordu.
    if (endless) {
      carding = performance.now() + 2600;
      els.cardTitle.textContent = `${I18n.t("endless")} · 1`;
      els.cardText.textContent = sonsuzKartMetni();
      els.chapterCard.hidden = false;
    }
    running = true;
  ...
  window.__devAynasi = {
    ...
    // Yalancı aynanın önüne koyar; simetri testi için panelin ortasını da verir.
    teleportToLiar(index = 0) {
      const id = [...liars][0];
      if (!id) return null;
      const horizontal = id[0] === "h";
      const [x, y] = id.slice(1).split(",").map(Number);
      ...
      p.y = horizontal ? g.ay + inside(y, size) : g.ay + CELL / 2;
      p.vx = 0; p.vy = 0;
      return { horizontal, ax: g.ax, ay: g.ay };
    },
    forceFlee() { fleeGiant(performance.now()); },
    saveNow: () => devamKaydet(),
    savedRun: () => devamOku(),
    ...
    // Yansıma fiziğini gerçek çizim geometrisi üzerinden denetler.
    // Her bölüm için: taşma yok, yarım figür yok, oyuncuyla çakışma yok,
    // simetri birebir (yalancı ayna hariç), bant dışına taşan çizim yok.
    reflectionCheck(kind = "giant") {
      const p = players[0];
      const hedef = kind === "liar" ? (() => {
        const id = [...liars][0];
        if (!id) return null;
        return { horizontal: id[0] === "h", x: +id.slice(1).split(",")[0], y: +id.slice(1).split(",")[1] };
      })() : giant;
      if (!hedef) return null;
      const g = segGeometry(hedef);
      const yatay = hedef.horizontal;
      const eskiX = p.x, eskiY = p.y;
      const sorunlar = [];
      let cizilen = 0;
      // Panel boyunca ve dik eksende tarama: 11 x 9 nokta.
      for (let i = 0; i <= 10; i++) {
        for (let j = 1; j <= 9; j++) {
          const t = -0.3 + i * 0.16;             // panel boyu (0..1 içi panel)
          const d = (j / 9) * (GIANT_REVEAL + 20); // düzleme uzaklık
          const yon = hedef.horizontal ? (hedef.y >= size ? -1 : 1) : (hedef.x >= size ? -1 : 1);
          p.x = yatay ? g.ax + t * CELL : g.ax + yon * d;
          p.y = yatay ? g.ay + yon * d : g.ay + t * CELL;
          const y = yansimaGeometrisi(p, hedef, kind, null);
          if (!y.cizilir) continue;
          cizilen++;
          // 1) simetri: panel boyunca oyuncuyla aynı hizada (yalancı hariç)
          const oyuncuEksen = yatay ? p.x : p.y;
          if (kind !== "liar" && Math.abs(y.eksen - oyuncuEksen) > 0.001) {
            sorunlar.push("simetri kaymis");
          }
          // 2) yarım figür yok: en az %35'i panelde
          if (y.gorunen < 0.349) sorunlar.push("yarim figur");
          // 3) figür bandın (cam derinliğinin) içinde tamamen duruyor
          const merkez = yatay ? y.ry : y.rx;
          const bandBas = yatay ? y.band.y : y.band.x;
          const bandBoy = yatay ? y.band.h : y.band.w;
          if (merkez - y.yariBoy < bandBas - 0.01 || merkez + y.yariBoy > bandBas + bandBoy + 0.01) {
            sorunlar.push("derinlikte kesilme");
          }
          // 4) oyuncuyla çakışma yok: yansıma gövdesi ile oyuncu yarıçapı ayrık
          const oyuncuDik = yatay ? p.y : p.x;
          if (Math.abs(merkez - oyuncuDik) < y.yariBoy + RADIUS * 0.85 - 0.01) {
            sorunlar.push("oyuncuyla cakisma");
          }
          // 5) yansıma camın arkasında (oyuncunun tarafında değil)
          if ((merkez - y.duzlem) * y.taraf > -0.01) sorunlar.push("cam onunde");
        }
      }
      p.x = eskiX; p.y = eskiY;
      return { cizilen, sorunlar: [...new Set(sorunlar)] };
    },
    band: () => bandOfRoom,
